...
class NoteViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        if owner_id and title and body:
            user = User.objects.get(id=owner_id)
            ...
            return Response({"message": "Owner_id, title and body are required to create a note"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def retrieve(self,request,pk):
        ...
        if not note:
            ...
            return Response(data={"detail":"Note not found"}, status=status.HTTP_404_NOT_FOUND)
        serializer = NoteSerializer(instance=note,many = False)
        return Response(serializer.data)
    ...
    def update(self, request, pk):
        ...
        if note:
            ...
            if owner_id and title and body:
                ...
                return Response({"detail": "Title , Body and Owner are required"}, status=status.HTTP_400_BAD_REQUEST)
        else:
